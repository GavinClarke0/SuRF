...
namespace surf {
    ...
    public:
        BitvectorRank() : basic_block_size_(0) {};
        ...
        position_t rank(position_t pos) const {
            ...
            return (rank_lut_[block_id]
                    + popcountLinear(bits_.get(), block_id * word_per_basic_block, offset + 1));
        }
        ...
        void serialize(char *&dst) const {
            ...
            memcpy(dst, bits_.get(), bitsSize());
            dst += bitsSize();
            memcpy(dst, rank_lut_.get(), rankLutSize());
            dst += rankLutSize();
            align(dst);
        }
        ...
        static BitvectorRank *deSerialize(char *&src) {
            ...
            bv_rank->bits_ = std::make_unique<word_t[]>(bv_rank->numWords());
            memcpy(bv_rank->bits_.get(), src, bv_rank->bitsSize());
            src += bv_rank->bitsSize();

            bv_rank->rank_lut_ = std::make_unique<position_t[]>(bv_rank->rankLutSize() / sizeof(position_t));
            memcpy(bv_rank->rank_lut_.get(), src, bv_rank->rankLutSize());
    ...
    private:
        ...
        std::unique_ptr<position_t[]> rank_lut_; //rank look-up table
    };
