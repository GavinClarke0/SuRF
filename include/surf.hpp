...
namespace surf {
    ...
    public:
        ...
        static SuRF *deSerialize(char *src) {
            SuRF *surf = new SuRF();
            surf->louds_dense_ = std::unique_ptr<LoudsDense>(LoudsDense::deSerialize(src));
            surf->louds_sparse_ = std::unique_ptr<LoudsSparse>(LoudsSparse::deSerialize(src));
    ...
    void SuRF::create(const std::vector<std::string> &keys,
        ...
        builder_->build(keys);
        louds_dense_ = std::make_unique<LoudsDense>(builder_);
        louds_sparse_ = std::make_unique<LoudsSparse>(builder_);
