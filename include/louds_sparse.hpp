...
namespace surf {
    ...
    public:
        ...
        static LoudsSparse *deSerialize(char *&src) {
            ...
            louds_sparse->level_cuts_ = std::make_unique<position_t[]>(louds_sparse->height_);
            memcpy(louds_sparse->level_cuts_.get(), src,
                   sizeof(position_t) * (louds_sparse->height_));
            src += (sizeof(position_t) * (louds_sparse->height_));
            align(src);
            louds_sparse->labels_ = std::unique_ptr<LabelVector>(LabelVector::deSerialize(src));
            louds_sparse->child_indicator_bits_ = std::unique_ptr<BitvectorRank>(BitvectorRank::deSerialize(src));
            louds_sparse->louds_bits_ = std::unique_ptr<BitvectorSelect>(BitvectorSelect::deSerialize(src));
            louds_sparse->suffixes_ = std::unique_ptr<BitvectorSuffix>(BitvectorSuffix::deSerialize(src));
            align(src);
            return louds_sparse;
        }
    ...
    LoudsSparse::LoudsSparse(const SuRFBuilder *builder) {
        ...
        louds_bits_ = std::make_unique<BitvectorSelect>(kSelectSampleInterval, builder->getLoudsBits(),
                                                        num_items_per_level, start_level_, height_);
