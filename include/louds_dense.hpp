...
namespace surf {
    ...
    public:
        ...
        void serialize(char *&dst) const {
            ...
            dst += sizeof(height_);
            memcpy(dst, level_cuts_.get(), sizeof(position_t) * height_);
        ...
        static LoudsDense *deSerialize(char *&src) {
            ...
            louds_dense->level_cuts_ = std::make_unique<position_t[]>(louds_dense->height_);
            memcpy(louds_dense->level_cuts_.get(), src,
                   sizeof(position_t) * (louds_dense->height_)); // TODO:

            src += (sizeof(position_t) * (louds_dense->height_));
            align(src);
            louds_dense->label_bitmaps_ = std::unique_ptr<BitvectorRank>(BitvectorRank::deSerialize(src));
            louds_dense->child_indicator_bitmaps_ = std::unique_ptr<BitvectorRank>(BitvectorRank::deSerialize(src));
            louds_dense->prefixkey_indicator_bits_ = std::unique_ptr<BitvectorRank>(BitvectorRank::deSerialize(src));
            louds_dense->suffixes_ = std::unique_ptr<BitvectorSuffix>(BitvectorSuffix::deSerialize(src));
            align(src);
            return louds_dense;
        }
    ...
    private:
        static constexpr position_t kNodeFanout = 256;
        static const position_t kRankBasicBlockSize;

        level_t height_;
        std::unique_ptr<position_t[]> level_cuts_; // position of the last bit at each level
    ...
    inline const position_t LoudsDense::kRankBasicBlockSize = 512;
    ...
    LoudsDense::LoudsDense(const SuRFBuilder *builder) {
        ...
        level_cuts_ = std::unique_ptr<position_t[]>(new position_t[height_]);
        ...
        child_indicator_bitmaps_ = std::make_unique<BitvectorRank>(kRankBasicBlockSize,
                                                                   builder->getBitmapChildIndicatorBits(),
                                                                   num_bits_per_level, 0, height_);
        prefixkey_indicator_bits_ = std::make_unique<BitvectorRank>(kRankBasicBlockSize,
        ...
        if (builder->getSuffixType() == kNone) {
            ...
            suffixes_ = std::make_unique<BitvectorSuffix>(builder->getSuffixType(),
