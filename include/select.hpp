...
namespace surf {
    ...
    public:
        BitvectorSelect() : sample_interval_(0), num_ones_(0) {};
        ...
        void serialize(char *&dst) const {
            ...
            dst += bitsSize();
            memcpy(dst, select_lut_.get(), selectLutSize());
            dst += selectLutSize();
            align(dst);
        }
        ...
        static BitvectorSelect *deSerialize(char *&src) {
            ...
            bv_select->bits_ = std::make_unique<word_t[]>(bv_select->numWords());
            memcpy(bv_select->bits_.get(), src, bv_select->bitsSize());
            src += bv_select->bitsSize();
            bv_select->select_lut_ = std::make_unique<position_t[]>(bv_select->selectLutSize() / sizeof(position_t));
            memcpy(bv_select->select_lut_.get(), src, bv_select->selectLutSize());
    ...
    private:
        ...
        std::unique_ptr<position_t[]> select_lut_; //select look-up table
    };
